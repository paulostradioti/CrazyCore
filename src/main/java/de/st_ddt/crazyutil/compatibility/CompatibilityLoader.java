...
public class CompatibilityLoader {
    ...
    static {
        ...
        } else if (serverClassName.equals("net.glowstone.GlowServer")) {
            implementation = "glowstone";
            version = "v" + server.getBukkitVersion().replaceFirst("-", "_").split("-", 2)[0].replaceAll("\\.", "_");
        } else {
            implementation = null;
            version = null;
        }
    ...
    /**
     * Removes all compatibility providers. Moves all files to the backup
     * folder.<br>
     * Should be called after plugin has been updated.
     *
     * @param plugin The plugin which compatibility folder should be cleared.
     */
